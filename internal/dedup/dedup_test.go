...
// setupMemoryDir creates a temp memory dir holding only metadata (.runtime/
// for WAL siblings). In v2 mistake CONTENT lives in native memory files, not a
// mistakes/ subdir — those are written under nativeDir(mem) by writeMistake.
func setupMemoryDir(t *testing.T) string {
	t.Helper()
	dir := t.TempDir()
	for _, sub := range []string{".runtime"} {
		if err := os.MkdirAll(filepath.Join(dir, sub), 0o755); err != nil {
			t.Fatal(err)
		}
	}
	if err := os.MkdirAll(nativeDir(dir), 0o755); err != nil {
		t.Fatal(err)
	}
	return dir
}

// nativeDir is where native mistake files live in the fixture — a directory
// distinct from MemoryDir (mirrors how native files live under
// ~/.claude/projects/<slug>/memory, not under ~/.claude/memory).
func nativeDir(mem string) string { return filepath.Join(mem, "native") }

// collectNativeMistakes lists the native fixture corpus so tests can pass it to
// Options.Files exactly as production resolves it via native.Collect.
func collectNativeMistakes(t *testing.T, mem string) []native.MemFile {
	t.Helper()
	files, err := native.List(nativeDir(mem))
	if err != nil {
...
// writeMistake writes a native mistake markdown file with the given root-cause.
...
func writeMistake(t *testing.T, memDir, slug, rootCause string, recurrence int) string {
	t.Helper()
	path := filepath.Join(nativeDir(memDir), slug+".md")
...
func runDedup(t *testing.T, mem, target string, stdin io.Reader) (Decision, string, string) {
	...
	opts := Options{
		MemoryDir: mem,
		Files:     collectNativeMistakes(t, mem),
...
func TestPretool_AllowNoSimilarExists(t *testing.T) {
	...
	decision, _, wal := runDedup(t, mem, filepath.Join(nativeDir(mem), "new-css.md"),
...
func TestPretool_BlockHighSimilarity(t *testing.T) {
	...
	newContent := "---\ntype: mistake\nroot-cause: \"" + rootCause + "\"\n---\n"
	decision, msg, wal := runDedup(t, mem, filepath.Join(nativeDir(mem), "duplicate.md"),
		strings.NewReader(newContent))
...
func TestPretool_CandidateMediumSimilarity(t *testing.T) {
	...
	decision, _, wal := runDedup(t, mem, filepath.Join(nativeDir(mem), "related-oauth.md"),
		strings.NewReader(newContent))
...
func TestPretool_MissingRootCauseAllows(t *testing.T) {
	...
	decision, _, wal := runDedup(t, mem, filepath.Join(nativeDir(mem), "bodyless.md"),
		strings.NewReader(newContent))
...
func TestPretool_BlockScalarMarkerAllows(t *testing.T) {
	...
	decision, _, _ := runDedup(t, mem, filepath.Join(nativeDir(mem), "scalar.md"),
		strings.NewReader(newContent))
	if decision != Allow {
...
func TestPretool_EmptyCorpusAllows(t *testing.T) {
	mem := setupMemoryDir(t) // native dir exists but empty

	newContent := "---\ntype: mistake\nroot-cause: \"perfectly reasonable description with enough characters\"\n---\n"
	decision, _, wal := runDedup(t, mem, filepath.Join(nativeDir(mem), "first.md"),
		strings.NewReader(newContent))
	if decision != Allow {
		t.Errorf("expected Allow against empty native corpus, got %v", decision)
	}
	if wal != "" {
		t.Errorf("expected no WAL against empty native corpus, got: %s", wal)
	}
}
...
func TestPosttool_MissingRootCauseAllows(t *testing.T) {
	...
	path := filepath.Join(nativeDir(mem), "orphan.md")
	content := "---\ntype: mistake\nstatus: active\n---\n\nJust a body.\n"
	if err := os.WriteFile(path, []byte(content), 0o644); err != nil {
		t.Fatal(err)
	}
...
func TestPosttool_ConcurrentMergesIncrementSerially(t *testing.T) {
	...
	// The native corpus passed to both runs contains ONLY the existing file,
	// so scanMistakes cannot pick one new file as another's best match (the new
	// files are not in Files). The two new files live in the same native dir as
	// the existing file so they pass the v2 boundary check. Both carry the same
	// root-cause as existing → both score 100% and enter the posttool merge
	// branch concurrently.
	existingFiles := collectNativeMistakes(t, mem)
	writeNewFile := func(name string) string {
		p := filepath.Join(nativeDir(mem), name+".md")
		body := "---\ntype: mistake\nstatus: active\nroot-cause: \"" + rootCause + "\"\nrecurrence: 0\n---\n\nBody.\n"
...
func TestRun_MissingNativeCorpusAllows(t *testing.T) {
	// MemoryDir exists but there is no native corpus at all (no native dir).
	mem := t.TempDir()
	newContent := "---\nroot-cause: \"some perfectly valid root cause description here\"\n---\n"
	decision, _, wal := runDedup(t, mem, filepath.Join(nativeDir(mem), "new.md"),
		strings.NewReader(newContent))
	if decision != Allow {
		t.Errorf("expected Allow, got %v", decision)
	}
...
// TestRun_RejectsTargetOutsideNativeDirs guards against a caller (or a
// poorly-written hook) handing dedup a path that lives outside any native
// memory directory. Without the v2 boundary check, dedup would happily read the
// outside file's root-cause line, run similarity against the native mistakes,
// and on a 100% match enter the posttool merge path — which deletes the outside
// file and writes a dedup-merged WAL event for content that never belonged in
// the memory store. External review 2026-05-08, finding E3 (the security
// property is preserved; only the boundary moved from MemoryDir to the native
// memory dirs derived from Options.Files).
func TestRun_RejectsTargetOutsideNativeDirs(t *testing.T) {
	mem := setupMemoryDir(t)
	...
	if decision != Allow {
		t.Errorf("expected Allow (target outside native dirs must be rejected), got %v", decision)
	}
	if wal != "" {
